...
set_polybar_values() {
	...
	if [[ $INTERFACE == en* || $INTERFACE == eth* ]]; then
		sed -i -e "s/sys_eth_interface = .*/sys_eth_interface = $INTERFACE/g" ${SYSFILE}
	elif [[ $INTERFACE == wl* ]]; then
		sed -i -e "s/sys_wlan_interface = .*/sys_wlan_interface = $INTERFACE/g" ${SYSFILE}
	fi
}
...
install_bspwm () {
	...
	sudo pacman -S --needed --noconfirm brightnessctl bspwm dmenu dunst feh mousepad mpc mpd ncmpcpp nm-connection-editor picom polybar rofi sddm slock sxhkd thunar thunar-archive-plugin thunar-media-tags-plugin thunar-volman \
