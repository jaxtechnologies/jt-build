...
#################################### To Do #####################################
################################################################################
# Copy Fish and Alacritty configs from CachyOS
# Create function to select shell - FISH or BASH or ZSH?
# chsh -s /usr/bin/fish
# chsh -s /usr/bin/bash
# chsh -s /usr/bin/zsh
# Timeshift?
...
INSTALL_PATH=$(pwd)
LOG=$INSTALL_PATH/jaxtech_base_setup.log
rm $LOG > /dev/null 2>&1
...
"
echo ""
echo " Jax|Tech LMDE Base Setup Script will launch in 5 seconds..."
...
update_base_system() {
    ...
    if ! command -v whiptail &> /dev/null; then
        ...
        sudo apt-get install whiptail -y 2>&1 | tee -a $LOG
    fi
...
install_base_apps() {
    clear
    sudo nala install alacritty curl dos2unix eza fish fonts-powerline gxmessage neovim openssh-server sxhkd vim xdotool xrdp -y 2>&1

    if id -nG xrdp | grep -qw ssl-cert; then
        echo "User xrdp already exists in ssl-cert group, skipping..." 2>&1 | tee -a $LOG
        sleep 3
    else
	...
	gsettings set org.cinnamon.desktop.background picture-uri 'file:///$HOME/Pictures/lmde7.jpg' 2>&1 | tee -a $LOG
...
devops() {
    ...
    checkboxes["python3-venv"]="python3-venv"
...
opt_conky() {
    ...
    echo "" 2>&1 | tee -a $LOG
    echo "" 2>&1 | tee -a $LOG
    echo "Select the type of system to install conky on..." 2>&1 | tee -a $LOG
    echo "" 2>&1 | tee -a $LOG
